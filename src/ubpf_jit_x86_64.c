...
static uint32_t translate(struct jit_state *state, riscv_t *rv, block_t *block)
{
    /* Entry point */
    ...
    for (idx = 0, ir = block->ir_head; idx < block->n_insn; idx++, ir = next) {
        ...
        case rv_insn_beq:
            ...
            // if (ir->branch_taken) {
            //     emit_jcc(state, 0x84, ir->pc + ir->imm);
            // } else {
            emit_load_imm(state, RAX, ir->pc + ir->imm);
            emit_store(state, S32, RAX, platform_parameter_registers[0],
                       offsetof(struct riscv_internal, PC));
            emit_jcc_offset(state, 0x84, 13);
            // }
...
uint32_t ubpf_translate_x86_64(riscv_t *rv, block_t *block)
{
    ...
    state->num_insn = 0;
    state->num_jumps = 0;
    uint32_t entry_loc = translate(&(*state), rv, block);
