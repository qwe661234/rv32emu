...
static inline void emit_jcc_offset(struct jit_state *state,
                                   int code,
                                   uint32_t jump_state_offset)
{
    emit1(state, 0x0f);
    emit1(state, code);
    emit_jump_target_offset(state, state->offset,
                            state->offset + jump_state_offset - 2);
    emit4(state, 0);
}
...
// static inline void emit_call(struct jit_state *state, void *target) {
//   /*
...
// #if defined(_WIN32)
//   /* Deallocate home register space + spilled register + alignment space - 5
//    * registers */
